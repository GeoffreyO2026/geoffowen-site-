...
function formatConversationForEmail(messages) {
  ...
  return messages.map(m => {
    const speaker = m.role === 'user' ? 'You' : 'The Biologically Aligned Parenting Bot';
...
async function generatePersonalizedNote(messages) {
  const transcript = messages.map(m => `${m.role === 'user' ? 'Parent' : 'Bot'}: ${m.content}`).join('\n\n');

  const prompt = `Below is a conversation between a parent and an AI bot trained on Geoff Owen's Biologically Aligned Parenting framework. Based on this conversation, do two things and respond with ONLY valid JSON, no other text:

1. Pick exactly one category from this list that best fits what the parent was asking about: ${CATEGORIES.map(c => `"${c}"`).join(', ')}
2. Write a short, warm note (3-4 sentences, in Geoff's measured, non-salesy voice) that will appear at the top of an email to this parent, above the full conversation transcript. Reference what they actually discussed, specifically, not generically. Do not repeat the closing Calendly sentence, that is added separately.
...
async function sendViaResend(email, note, transcriptText) {
  const html = `
    <div style="font-family: Georgia, serif; color: #1a1a1a; max-width: 600px; margin: 0 auto; line-height: 1.8;">
      <p style="color:#B8972A; font-family: Arial, sans-serif; font-size: 11px; letter-spacing: 0.1em; text-transform: uppercase; margin-bottom: 24px;">The Biologically Aligned Parenting Bot</p>
      ...
      <hr style="border: none; border-top: 1px solid #ddd; margin: 28px 0;">
      <p style="font-family: Arial, sans-serif; font-size: 15px; color: #1a1a1a; margin-bottom: 16px;">If you'd like to go deeper on this, my free guide is a good next step: <a href="https://geoffreyowen.com/guide" style="color:#B8972A;">https://geoffreyowen.com/guide</a></p>
      <p style="font-family: Arial, sans-serif; font-size: 13px; color: #888;">Or if you'd rather talk it through directly, you're welcome to book a free 45-minute call: <a href="https://calendly.com/geoffreyowen/45mins" style="color:#B8972A;">https://calendly.com/geoffreyowen/45mins</a></p>
    </div>
  `;
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to: [email],
      subject: 'Your conversation with the Biologically Aligned Parenting Bot',
      html: html
    })
  });
