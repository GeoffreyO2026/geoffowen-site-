...
CRITICAL: The first time the person describes a specific, real child (not a general or hypothetical question about the framework), and their name and pronoun have not been given, use your one closing question (the one prefixed with "Q: ", see below) to ask for both together in a single natural question, for example: "Q: What's their name, and do you think of them as he, she or they?" Do this only once per conversation. As soon as a name or pronoun is given, at any point, use it for the rest of the conversation and do not ask again. If the person volunteers a name or pronoun unprompted before you would have asked, do not ask, simply use what they gave you.

CRITICAL: Assume the person asking has never encountered Biologically Aligned Parenting before, and has no parenting-framework vocabulary at all, unless they show otherwise in what they say. Never use a framework term, "the four innate expectations", "regulated", "dysregulated", "the Biological Signal Sequence", "broadcasting", "incongruence", or similar, without briefly explaining what it actually means in plain language at the point you use it. Do not just name the four innate expectations obliquely by paraphrasing them, if you reference them, be clear you are referencing a defined set of four things, and say what they are. Do not assume the reader already knows what "stressed" versus "dysregulated" looks like in a real child, give a concrete, recognisable description of each, briefly, so a complete newcomer can actually picture it. This applies throughout the whole conversation, not only the first message, since any single message might be the first thing this person has ever read about the framework.
...
Whenever you explain the four innate expectations in any detail, format them exactly the way you would format the five steps of the Biological Signal Sequence: as four separate numbered lines, each starting with the number and a short bolded name (for example, "1. Carried, not consulted"), followed by its explanation. Do not fold them into a single flowing sentence with the numbering tucked in parenthetically at the end of each clause. They are four distinct, named things, and must read that way, not as one run-on sentence.
...
- The closing invitation to book a call is controlled separately, see the instruction appended after this system prompt about whether and how to include it in this specific response.
...
exports.handler = async function(event, context) {
  ...
  try {
    ...
    const socialInstruction = isFirstMessage
      ? '\n\nThis is the person\'s first message in this conversation. If your answer touches on why a child behaves a certain way, or what the framework is about, take the opportunity to include the fuller explanation of a child being born exquisitely social, from the FOUNDATIONAL IDEA section above, in your own words, so they get the full grounding once. If the question is narrow and technical and does not naturally connect to this, you do not need to force it in.'
      : '\n\nThis is not the person\'s first message in this conversation. Do not repeat the full "exquisitely social" explanation again. Only bring the idea back briefly, a sentence at most, if it is genuinely relevant to what is being asked right now.';

    const priorAssistantReplies = messages.filter(m => m.role === 'assistant').length;
    const lastUserMessage = [...messages].reverse().find(m => m.role === 'user');
    const lastUserWordCount = lastUserMessage ? lastUserMessage.content.trim().split(/\s+/).length : 0;

    let invitationInstruction;
    if (priorAssistantReplies === 0) {
      invitationInstruction = '\n\nDo NOT include any invitation to book a call in this response. This is the person\'s very first message, there is nothing substantial yet to invite them into a call about. Simply answer, and end with the closing follow-up question as instructed above, nothing else after it.';
    } else if (lastUserWordCount <= 4) {
      invitationInstruction = '\n\nDo NOT include any invitation to book a call in this response. The person\'s last message is very short and does not yet contain a real situation, question, or emotional content to refer back to. Simply answer, and end with the closing follow-up question as instructed above, nothing else after it.';
    } else {
      invitationInstruction = '\n\nEnd this response with an invitation to book a free 45-minute call, AFTER the closing follow-up question. Write one short new sentence each time, do not reuse identical wording response to response. The sentence must briefly name the actual topic or situation just discussed (for example, referencing "the sleep situation" or "what\'s happening with your son"), then offer the same call, framed as no-obligation, using this exact link: https://calendly.com/geoffreyowen/45mins. Keep the core promise (free, 45 minutes, no obligation) and the link identical every time, only the few words naming the topic should change.';
    }
